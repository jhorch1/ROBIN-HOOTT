...
import { User, Trophy, Star, Activity, Settings, Gamepad2, Rocket, Search } from "lucide-react";
...
export default function Dashboard() {
  ...
  const isDocente = usuario?.role === "DOCENTE" || usuario?.role === "ADMIN" || usuario?.rol === "DOCENTE" || usuario?.rol === "ADMIN";
  const [ranking, setRanking] = useState<RankingItem[]>([]);
  const [rankingMaraton, setRankingMaraton] = useState<RankingItem[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [loadingMaraton, setLoadingMaraton] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [errorMaraton, setErrorMaraton] = useState<string | null>(null);
  const [showMaraton, setShowMaraton] = useState<boolean>(false);
  ...
  // Manejo de Desafíos
  const [buscandoDuelo, setBuscandoDuelo] = useState<boolean>(false);
  const [prefillPin, setPrefillPin] = useState<string>("");

  // Manejo de Perfil
  const [editProfileOpen, setEditProfileOpen] = useState<boolean>(false);
  const [editNombre, setEditNombre] = useState<string>(usuario?.nombre || "");
  const [actualizandoPerfil, setActualizandoPerfil] = useState<boolean>(false);
  ...
  useEffect(() => {
    const onDuelFound = ({ pin }: { pin: string }) => {
    ...
    const onDuelError = ({ message }: { message: string }) => {
      setBuscandoDuelo(false);
      alert("Error al buscar duelo: " + message);
    };

    const onMarathonReady = () => {
    };
    
    const onMarathonError = ({ message }: { message: string }) => {
  ...
  return (
    ...
    >
      ...
      <div className="dashboard-main-grid">
        {/* Info del usuario */}
        <CustomCard icon={<User size={32} />} title="Perfil Academico" variant="default">
          <div style={{ display: "flex", flexDirection: "column", gap: "15px", marginTop: "20px" }}>
            ...
            <p style={{ fontSize: "1.1rem" }}><strong>Rol:</strong> {(usuario?.role as string) || (usuario?.rol as string) || "Estudiante"}</p>
      ...
      <div className="dashboard-challenges-grid">
        <CustomCard variant="red" icon={<Gamepad2 size={32} />} title="Desafio Clasico">
          ...
          <MyButton variant="danger" fullWidth style={{ padding: "16px" }} onClick={handleDesafioClasico}>¡JUGAR YA!</MyButton>
        </CustomCard>
